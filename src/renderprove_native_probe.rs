...
pub fn parse_renderprove_worker_image_observation(
    ...
    Ok(RenderproveWorkerImageObservation { image_id })
}
...
mod tests {
    ...
    fn plans_only_direct_descriptor_bound_podman_operations() {
        ...
        let observed =
            parse_renderprove_worker_image_observation(&format!("{}\n", digest("ef").as_str()))
                .expect("observation");
    ...
    fn image_observation_and_aliases_are_strictly_bounded() {
        for invalid in ["", "not-a-digest", "sha256:00\nsha256:11", "sha256:00\r"] {
            assert!(parse_renderprove_worker_image_observation(invalid).is_err());
        }
        assert!(
