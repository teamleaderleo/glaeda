...
//! A child domain is bound to one exact parent claim, may track only a subset of that claim, and
//! may budget no more than the parent owns in each tracked dimension.
//!
...
impl CapacityDomain {
    ...
    pub(crate) fn child(
        ...
        if !budget
            .tracked_dimensions()
            .is_subset_of(parent_claim.resources().tracked_dimensions())
        {
            return Err(ExecutionCapacityError::new(
                "budget.tracked_dimensions",
        ...
        if budget
            ...
            .any(|dimension| {
                budget
                    .amount(*dimension)
                    .expect("tracked child budget dimension must have an amount")
                    > parent_claim
                        .resources()
                        .amount(*dimension)
                        .expect("tracked parent claim dimension must have an amount")
            })
        {
            return Err(ExecutionCapacityError::new(
                "budget.amounts",
...
mod tests {
    ...
    #[test]
    fn child_domain_rejects_untracked_parent_dimensions_even_at_zero() {
        ...
        let error = CapacityDomain::child(
            domain_id('c'),
            generation(1),
            amounts(&[
        ...
        .unwrap_err();
        assert_eq!(error.code(), "child_dimension_set_not_subset");
    }

    #[test]
    fn child_budget_cannot_exceed_parent_and_exact_equality_is_valid() {
        let parent_domain = root(
            'd',
            8,
            amounts(&[
                ...
                (CapacityDimension::DiskBytes, 50 * GIB),
            ]),
        );
        let parent = claim(
            '1',
        ...
        let equal = CapacityDomain::child(
            domain_id('e'),
            generation(1),
            amounts(&[
                (CapacityDimension::CpuMillis, 4_000),
        ...
        .unwrap();
        assert_eq!(equal.parent_claim(), Some(&parent));
        assert_eq!(
            equal.budget().amount(CapacityDimension::CpuMillis),
            Some(4_000)
        );
    }
    ...
    #[test]
    fn child_claims_consume_child_budget_without_double_charging_parent() {
        let host = root(
            'f',
            9,
            amounts(&[
        ...
        let child = CapacityDomain::child(
            domain_id('a'),
            generation(3),
            amounts(&[
                (CapacityDimension::CpuMillis, 6_000),
        ...
        assert!(matches!(
            admit_capacity_claim(&child, &[task_one], &task_two),
            CapacityAdmissionDecision::Accepted { .. }
        ));
        assert_eq!(
            CapacityDomain::child(
    ...
    fn refusal_precedence_is_global_and_order_independent() {
        ...
        let duplicate = claim('4', &target, amounts(&[(CapacityDimension::CpuMillis, 1)]));
        assert_eq!(
            reason(&admit_capacity_claim(&target, &[full], &duplicate)),
