...
pub fn acquire_renderprove_protected_mount_lease(
    ...
    let project_alias = match create_alias_directory(&mount_root, "project") {
        Ok(alias) => alias,
        Err(error) => return Err(fail_acquisition(error, evidence_source, |_| {})),
    };
    let evidence_alias = match create_alias_directory(&mount_root, "evidence") {
        Ok(alias) => alias,
        Err(error) => {
            return Err(fail_acquisition(error, evidence_source, |first_error| {
                record_cleanup_error(
                    first_error,
                    remove_alias_directory(&mount_root, &project_alias.name, &project_alias.base),
                );
            }));
        }
    };
    ...
    if let Err(error) = attach_mount_pair(
        ...
        return Err(fail_acquisition(error, evidence_source, |first_error| {
            record_cleanup_error(
                first_error,
                remove_alias_directory(&mount_root, &evidence_alias.name, &evidence_alias.base),
            );
            record_cleanup_error(
                first_error,
                remove_alias_directory(&mount_root, &project_alias.name, &project_alias.base),
    ...
    let project_alias_mount = match open_alias_mount(
        ...
        Err(error) => {
            return Err(fail_acquisition(error, evidence_source, |first_error| {
                record_cleanup_error(first_error, detach_mount(&evidence_alias.path));
                record_cleanup_error(first_error, detach_mount(&project_alias.path));
                record_cleanup_error(
                    first_error,
                    remove_alias_directory(&mount_root, &evidence_alias.name, &evidence_alias.base),
                );
...
fn fail_acquisition(
    primary: RenderproveProtectedMountError,
    evidence: PreparedEvidenceDirectory,
    cleanup: impl FnOnce(&mut Option<RenderproveProtectedMountError>),
) -> RenderproveProtectedMountError {
    let mut first_error = None;
    cleanup(&mut first_error);
    record_cleanup_error(&mut first_error, evidence.rollback());
    first_error.unwrap_or(primary)
}
...
fn attach_mount_pair<B: MountOperations>(
    ...
    let evidence_mount = match backend.clone_mount(evidence_source) {
        Ok(mount) => mount,
        Err(error) => {
            if backend.detach_mount(project_alias_path).is_err() {
                return Err(RenderproveProtectedMountError::new(
                    RenderproveProtectedMountErrorKind::CleanupFailed,
                    "mount_rollback",
                    "partial Renderprove mount acquisition could not be rolled back",
                ));
            }
            return Err(error);
        }
...
impl PreparedEvidenceDirectory {
    ...
    fn rollback(mut self) -> Result<(), RenderproveProtectedMountError> {
    ...
    fn rollback_internal(&mut self) -> Result<(), RenderproveProtectedMountError> {
        let mut first_error = None;
        for created in self.created.iter().rev() {
            record_cleanup_error(&mut first_error, remove_created_evidence_directory(created));
        }
        match first_error {
            Some(error) => Err(error),
            None => Ok(()),
...
fn prepare_relative_directory(
    ...
    let preparation = (|| -> Result<(), RenderproveProtectedMountError> {
        for component in components {
    ...
    match preparation {
        Ok(()) => Ok(prepared),
        Err(primary) => match prepared.rollback() {
            Ok(()) => Err(primary),
...
fn require_root_owned_mount_root(root: &OwnedFd) -> Result<(), RenderproveProtectedMountError> {
    ...
    if !valid_mount_root_metadata(stat.st_uid, stat.st_gid, stat.st_mode) {
        return Err(RenderproveProtectedMountError::new(
            RenderproveProtectedMountErrorKind::UnsafeFilesystem,
            "mount_root",
            "Renderprove protected mount root must be exact root-owned non-listable mode 0701",
        ));
    }
    Ok(())
}

fn valid_mount_root_metadata(uid: u32, gid: u32, mode: u32) -> bool {
    uid == 0 && gid == 0 && mode & 0o7777 == 0o701
}
...
fn validate_absolute_path(path: PathBuf) -> Result<PathBuf, RenderproveProtectedMountError> {
    ...
    let normalized = path.to_str().is_some()
        && bytes.len() > 1
        && bytes.first() == Some(&b'/')
        && !bytes.ends_with(b"/")
        && !bytes.contains(&0)
...
mod tests {
    ...
    #[test]
    fn mount_root_is_non_listable_but_runner_traversable() {
        assert!(valid_mount_root_metadata(0, 0, 0o040701));
        for (uid, gid, mode) in [
            (1, 0, 0o040701),
            (0, 1, 0o040701),
            (0, 0, 0o040700),
            ...
            assert!(!valid_mount_root_metadata(uid, gid, mode));
        }
    }

    #[test]
    fn evidence_paths_require_exact_normalized_relative_bytes() {
        for invalid in [
            "",
            "/evidence",
            "./evidence",
            ...
            assert!(
                relative_components(Path::new(invalid)).is_err(),
                "{invalid}"
            );
        }
        assert_eq!(
            relative_components(Path::new("artifacts/renderprove")).expect("valid evidence path"),
            [OsStr::new("artifacts"), OsStr::new("renderprove")]
        );
    }
    ...
    #[test]
    fn attached_pair_surfaces_rollback_failure() {
        ...
        assert_eq!(error.stage(), "mount_rollback");
    }
    ...
    #[test]
    fn partial_acquisition_surfaces_alias_removal_failure() {
        let aliases = TempRoot::new("alias-removal-failure");
        let alias_root = aliases.open();
        let alias_name = OsString::from("project-33333333333333333333333333333333");
        let alias =
            create_alias_directory_with_generator(
                &alias_root,
                "project",
                || Ok(alias_name.clone()),
            )
            .expect("create alias");
        std_fs::write(aliases.0.join(&alias.name).join("occupied"), b"occupied")
            .expect("occupy alias");

        let project = TempRoot::new("alias-removal-evidence");
        let project_fd = project.open();
        let evidence = prepare_relative_directory(&project_fd, Path::new("artifacts/renderprove"))
            .expect("prepare evidence");
        let primary = RenderproveProtectedMountError::new(
            RenderproveProtectedMountErrorKind::Io,
            "mount",
            "injected acquisition failure",
        );
        let error = fail_acquisition(primary, evidence, |first_error| {
            record_cleanup_error(
                first_error,
                remove_alias_directory(&alias_root, &alias.name, &alias.base),
            );
        });
        assert_eq!(
            error.kind(),
    ...
    #[test]
    fn evidence_rollback_failure_is_explicit() {
        let root = TempRoot::new("evidence-rollback-failure");
        let project = root.open();
        let prepared = prepare_relative_directory(&project, Path::new("artifacts/renderprove"))
            .expect("prepare evidence");
        std_fs::write(
            root.0.join("artifacts/renderprove/private.txt"),
            b"occupied",
        )
        .expect("occupy evidence directory");
        let error = prepared
            .rollback()
            .expect_err("nonempty rollback must fail");
        assert_eq!(
            error.kind(),
            RenderproveProtectedMountErrorKind::CleanupFailed
        );
        assert_eq!(error.stage(), "evidence_rollback");
    }
