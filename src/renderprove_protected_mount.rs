...
use rustix::mount::{
    self, MoveMountFlags, OpenTreeFlags, UnmountFlags,
};
...
use crate::renderprove_verification::{
    RenderproveSourceIdentity, RenderproveVerificationRequest,
};
...
impl RenderproveProtectedMountLease {
    ...
    pub fn cleanup(mut self) -> Result<RenderproveProtectedMountCleanupReceipt, RenderproveProtectedMountError> {
...
pub fn acquire_renderprove_protected_mount_lease(
    ...
    let mount_root = open_absolute_directory(mount_root_path, "mount_root")
    ...
    let evidence_source = open_or_create_relative_directory(
        &project_source,
...
fn create_alias_directory(
    mount_root: &OwnedFd,
    prefix: &str,
    root_identity: &ReviewedFilesystemIdentity,
) -> Result<CreatedAlias, RenderproveProtectedMountError> {
    create_alias_directory_with_generator(mount_root, prefix, root_identity, || {
        generate_alias_name(prefix)
    })
}
    ...
    for _ in 0..MAX_ALIAS_ATTEMPTS {
        ...
        match fs::mkdirat(mount_root.as_fd(), &name, PRIVATE_DIRECTORY_MODE) {
            Ok(()) => {
                ...
                let base_identity = inspect_directory_identity(&base, "alias")?;
                let root_stat = fs::fstat(mount_root).map_err(|_| RenderproveProtectedMountError::new(
                    RenderproveProtectedMountErrorKind::Io,
                    "alias",
                    "Renderprove mount root identity could not be inspected",
                ))?;
                let base_stat = fs::fstat(&base).map_err(|_| RenderproveProtectedMountError::new(
                    RenderproveProtectedMountErrorKind::Io,
                    "alias",
                    "Renderprove mount alias identity could not be inspected",
                ))?;
                if base_stat.st_uid != root_stat.st_uid || base_stat.st_gid != root_stat.st_gid {
                    let _ = fs::unlinkat(mount_root.as_fd(), &name, AtFlags::REMOVEDIR);
                    return Err(RenderproveProtectedMountError::new(
                        RenderproveProtectedMountErrorKind::UnsafeFilesystem,
                        "alias",
                        "Renderprove mount alias ownership is unsafe",
                    ));
                }
                let _ = root_identity;
...
fn map_mount_error(error: Errno) -> RenderproveProtectedMountError {
    match error {
        ...
        Errno::ACCES | Errno::PERM => RenderproveProtectedMountError::new(
            RenderproveProtectedMountErrorKind::PermissionDenied,
            "mount",
...
fn open_absolute_directory(
    ...
    for component in components {
        current = fs::openat(current.as_fd(), component, DIRECTORY_FLAGS, Mode::empty())
...
fn open_or_create_relative_directory(
    ...
    let mut current = io::dup(project).map_err(|_| RenderproveProtectedMountError::new(
        RenderproveProtectedMountErrorKind::Io,
        "evidence",
        "reviewed project descriptor could not be retained for evidence traversal",
    ))?;
    for component in components {
        current = match fs::openat(current.as_fd(), component, DIRECTORY_FLAGS, Mode::empty()) {
            Ok(directory) => directory,
            Err(Errno::NOENT) => {
                fs::mkdirat(current.as_fd(), component, PRIVATE_DIRECTORY_MODE).map_err(|error| {
                    match error {
                ...
                fs::openat(current.as_fd(), component, DIRECTORY_FLAGS, Mode::empty()).map_err(|_| {
                    RenderproveProtectedMountError::new(
                        RenderproveProtectedMountErrorKind::UnsafeFilesystem,
                        "evidence",
    ...
    inspect_directory_identity(&current, "evidence")?;
    Ok(current)
}
...
fn inspect_directory_identity(
    ...
    let stat = fs::fstat(descriptor).map_err(|_| RenderproveProtectedMountError::new(
        RenderproveProtectedMountErrorKind::Io,
        stage,
...
mod tests {
    ...
    impl TempRoot {
        ...
        fn open(&self) -> OwnedFd {
            fs::open(&self.0, OFlags::PATH | OFlags::DIRECTORY | OFlags::CLOEXEC, Mode::empty())
    ...
    fn source_paths_are_absolute_normalized_and_private() {
        let identity = ReviewedFilesystemIdentity::new(1, 2, 1000, 1000, 0o700)
            .expect("identity");
        for invalid in ["relative", "/", "/tmp/./project", "/tmp/../project"] {
            assert!(RenderproveProtectedMountSource::new(
                source_identity(),
                invalid,
                identity.clone(),
            )
    ...
    fn alias_collision_retries_without_using_caller_selected_names() {
        ...
        let identity = inspect_directory_identity(&root_fd, "test").expect("identity");
        let collision = OsString::from("project-00000000000000000000000000000000");
        fs::mkdirat(root_fd.as_fd(), &collision, PRIVATE_DIRECTORY_MODE)
