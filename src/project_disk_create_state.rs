...
use crate::project_disk_host_observation::{
    LimaStandaloneDiskName, ProjectDiskBackingIdentity, ProjectDiskPhysicalIdentity,
};
...
impl ProjectDiskCreateSourceIdentity {
    #[cfg(test)]
    const fn from_digest(digest: Sha256Digest) -> Self {
        Self(digest)
    }
...
impl ProjectDiskCreateRecord {
    ...
    pub fn record_create_success(
        ...
        let binding = ProjectDiskCreatedUnformattedBinding {
            ...
            backing_identity: physical.backing_identity.clone(),
...
impl ProjectDiskCreatePhysicalProof {
    fn confirm(
        ...
        if self.project != record.project
            ...
            || self.disk_name != record.disk_name
            || self.attempt_generation != attempt_generation
            || self.backing_identity.digest().as_str()
...
mod tests {
    ...
    use crate::project_catalog::ProjectIdentity;
    use crate::project_disk_host_observation::{
        ProjectDiskBackingIdentity, ProjectDiskPhysicalIdentity,
    };
    use crate::project_disk_lease::{ProjectDiskGeneration, ProjectDiskId};
    ...
    fn backing(byte: char) -> ProjectDiskBackingIdentity {
        ProjectDiskBackingIdentity::parse(digest(byte).as_str()).unwrap()
    }
    ...
    fn create_started_requires_prior_durable_authorization() {
        ...
        let start = authorized.plan_create_start().unwrap();
        assert_eq!(
            start.requested_logical_bytes(),
            accepted.requested_logical_bytes()
        );
        let started = authorized.record_create_started(&start).unwrap();
        assert!(matches!(
            started.state(),
    ...
    fn uninterrupted_success_requires_executor_and_physical_proof() {
        ...
        assert_eq!(binding.backing_identity(), &backing('c'));
    }
