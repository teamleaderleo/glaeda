        ...
        let (intent, resulting_record) = match previous {
            ...
            Some(previous) => {
                ...
                if same_semantic_event(previous, &next_record) {
                    let mut replay_record = previous.clone();
                    replay_record.last_observed_at = next_record.last_observed_at;
                    (
...
fn validate_transition(
    ...
    let start_time_drift = match (&previous.phase, &next.phase) {
        (
            GitHubWorkflowJobPhase::InProgress {
                started_at: previous_started_at,
            },
            GitHubWorkflowJobPhase::Completed {
                started_at: Some(next_started_at),
                ..
            },
        ) => previous_started_at != next_started_at,
        (
            GitHubWorkflowJobPhase::InProgress { .. },
            GitHubWorkflowJobPhase::Completed {
                started_at: None, ..
    ...
    if start_time_drift {
        return Err(GitHubWorkflowJobMapperError::new(
            "started_at",
            "start_time_drift",
            "GitHub workflow-job start time must remain exact through completion and replay",
        ));
    }
...
mod tests {
    ...
    fn exact_duplicate_delivery_is_a_no_op() {
        ...
        assert_eq!(
            replay.resulting_record().phase(),
            first.resulting_record().phase()
        );
        assert_eq!(replay.resulting_record().last_observed_at, time(1_060));
    }
    ...
    #[test]
    fn exact_replay_advances_the_observation_barrier() {
        let mapper = mapper_with_names("Verify", "verify");
        let first = mapper
            .map(
                &event(GitHubWorkflowJobAction::Queued, 1_050),
            ...
            .expect("first queued mapping");
        let replay = mapper
            .map(
                &event(GitHubWorkflowJobAction::Queued, 1_200),
                Some(first.resulting_record()),
                time(1_200),
            )
            .expect("exact replay");
        assert!(matches!(
        ...
        assert_eq!(replay.resulting_record().last_observed_at, time(1_200));

        let older_progress = event(GitHubWorkflowJobAction::InProgress, 1_100);
        assert_eq!(
            mapper
                .map(
                    &older_progress,
                    Some(replay.resulting_record()),
                    time(1_200),
                )
                .expect_err("event older than processed replay")
                .code,
            "event_observation_reversal"
        );
    }
    ...
    #[test]
    fn completed_phase_retains_and_validates_exact_start_time() {
        let mapper = mapper_with_names("Verify", "verify");
        let running = mapper
            .map(
                &event(GitHubWorkflowJobAction::InProgress, 1_100),
            ...
            .expect("running mapping");

        let mut drifted_definition = definition(GitHubWorkflowJobAction::Completed, 1_200);
        drifted_definition.started_at = Some(time(1_150));
        let drifted = GitHubWorkflowJobEvent::new(drifted_definition).expect("drifted completion");
        assert_eq!(
            mapper
                .map(&drifted, Some(running.resulting_record()), time(1_200))
                .expect_err("completion start drift")
                .code,
            "start_time_drift"
        );
        ...
        let completed = mapper
            ...
            .expect("completed mapping");
        assert!(matches!(
            completed.resulting_record().phase(),
            GitHubWorkflowJobPhase::Completed {
        ...
        let mut replay_drift_definition = definition(GitHubWorkflowJobAction::Completed, 1_250);
        replay_drift_definition.started_at = Some(time(1_150));
        let replay_drift =
            GitHubWorkflowJobEvent::new(replay_drift_definition).expect("drifted completed replay");
        assert_eq!(
            mapper
                .map(
                    &replay_drift,
                    Some(completed.resulting_record()),
                    time(1_250),
                )
                .expect_err("completed replay start drift")
                .code,
            "start_time_drift"
        );
    }
