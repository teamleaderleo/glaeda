...
macro_rules! define_operator_errors {
    (
        ...
        const fn operator_error_spec(code: OperatorErrorCode) -> OperatorErrorSpec {
            match code {
                $(
                    OperatorErrorCode::$code => OperatorErrorSpec::new(
                        $summary,
...
mod tests {
    ...
    fn every_code_has_one_unique_complete_mapping() {
        ...
        for &code in ALL_OPERATOR_ERROR_CODES {
            ...
            assert!(encoded_codes.insert(encoded_code), "duplicate code: {code:?}");
            assert_eq!(public.schema_version(), OPERATOR_PUBLIC_ERROR_SCHEMA_VERSION);
            ...
            if public.remediation() == OperatorRemediationClass::Dependency {
                assert!(public.dependency().is_some(), "missing dependency: {code:?}");
