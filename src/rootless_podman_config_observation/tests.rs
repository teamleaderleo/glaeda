...
use super::{
    ...
    RunnerConfigIdentity, TrustedConfigFile, TrustedConfigFileProblem, observe_with, render_human,
};
...
fn source(
    report: &super::RootlessPodmanConfigObservationReport,
    role: RootlessPodmanConfigSourceRole,
) -> &super::RootlessPodmanConfigSourceObservation {
    report
        .sources
        .iter()
...
fn unknown_runner_source_hides_lower_precedence_values() {
    ...
    assert!(report.resolved.containers.network_backend.value.is_none());
}
...
fn reports_never_serialize_raw_configuration_or_os_errors() {
    ...
    filesystem.files.insert(
        "/etc/containers/containers.conf".into(),
        TrustedConfigFile::Present(b"[containers]\nsecret_material = \"DO-NOT-LEAK\"\n".to_vec()),
    );
    filesystem.files.insert(
        "/var/lib/project-runner/.config/containers/storage.conf".into(),
