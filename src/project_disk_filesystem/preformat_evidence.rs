...
impl ProjectDiskPreFormatEvidence {
    ...
    /// changed, if any required blank/unused predicate is false, or if the bound or observed block
    /// node identity is absent or does not match exactly.
    pub fn confirm(
        &self,
        plan: &ProjectDiskFilesystemFormatPlan,
        physical_identity_digest: &Sha256Digest,
        backing_identity_digest: &Sha256Digest,
        observed_block_device_rdev: u64,
        observed_block_device_inode: u64,
        ...
        if plan.filesystem().project() != &self.summary.project
            ...
            || self.block_device_rdev == 0
            || self.block_device_inode == 0
            || self.block_device_rdev != observed_block_device_rdev
            || self.block_device_inode != observed_block_device_inode
...
impl fmt::Debug for ProjectDiskPreFormatEvidence {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("block_device_identity", &"<redacted>")
...
mod tests {
    ...
    fn exact_blank_unused_device_confirms_format_plan() {
        let plan = plan(7);
        let evidence =
            ProjectDiskPreFormatEvidence::for_test(&plan, 2049, 77, digest('a'), digest('b'));
        evidence
            .confirm(&plan, &digest('a'), &digest('b'), 2049, 77)
            .unwrap();
    ...
    #[test]
    fn wrong_physical_or_backing_identity_is_rejected_atomically() {
        let plan = plan(7);
        let evidence =
            ProjectDiskPreFormatEvidence::for_test(&plan, 2049, 77, digest('a'), digest('b'));
        assert!(
            evidence
                .confirm(&plan, &digest('9'), &digest('b'), 2049, 77)
                .is_err()
        );
        assert!(
            evidence
                .confirm(&plan, &digest('a'), &digest('8'), 2049, 77)
                .is_err()
        );
    }

    #[test]
    fn wrong_or_empty_block_node_identity_is_rejected_atomically() {
        let plan = plan(7);
        let evidence =
            ProjectDiskPreFormatEvidence::for_test(&plan, 2049, 77, digest('a'), digest('b'));
        assert!(
            evidence
                .confirm(&plan, &digest('a'), &digest('b'), 2050, 77)
                .is_err()
        );
        assert!(
            evidence
                .confirm(&plan, &digest('a'), &digest('b'), 2049, 78)
                .is_err()
        );
        assert!(
            ProjectDiskPreFormatEvidence::for_test(&plan, 2049, 0, digest('a'), digest('b'))
                .confirm(&plan, &digest('a'), &digest('b'), 2049, 0)
                .is_err()
        );
        assert!(
            ProjectDiskPreFormatEvidence::for_test(&plan, 0, 77, digest('a'), digest('b'))
                .confirm(&plan, &digest('a'), &digest('b'), 0, 77)
                .is_err()
        );
    }
    ...
    fn another_filesystem_generation_or_plan_is_rejected() {
        ...
        let second = plan(8);
        let evidence =
            ProjectDiskPreFormatEvidence::for_test(&first, 2049, 77, digest('a'), digest('b'));
        assert!(
            evidence
                .confirm(&second, &digest('a'), &digest('b'), 2049, 77)
                .is_err()
        );
    }
