...
- [x] Retain the canonical store lock from current-capacity derivation through the bounded Scale Set poll and exact response publication.
- [x] Add a live persistence-aware Scale Set clone-admission source; an earlier zero-capacity idle poll remains advisory and cannot authorize a later mutation.
...
- [x] Persist the controller's freshly observed-absent decision as versioned `CloneAuthorized` state under the catalog lock before cloning; use clone-with-start and discard only a stopped partial clone inside that authorized creation window, avoiding Lima's create-or-start `start NAME` behavior. Schema-v1 attempts and the legacy `Provisioning` phase fail closed. The later clone-start transaction independently revalidates absence before mutation.
...
- [x] Implement the root-only boot activation transaction: verify the installed PF attachment and
  exact anchor, load the canonical anchor before enforcement, delegate live rule ordering and
