...
`onboard` runs `preflight`, `fix`, the sudo-plans (`--sudo wait`, the default, prints them and waits for the operator to run `sudo-plan --run`; `--sudo run` runs them itself over `ssh -t`; `--sudo stop` stops those hosts there), `fix` again, and a fresh preflight. Every host still ready is then enrolled in parallel, with a heartbeat line every 30 seconds. A runner is registered on every eligible host that has none, using one org registration token minted on this Mac (`gh api orgs/<runner.org>/actions/runners/registration-token`) and passed over stdin to `persistent-compile up`, never in argv or a log. It ends with one table: host, node id, step reached, result, log. Hosts come from the manifest: those with a `node_id` whose class does not set `compile_lane: false` (16 GB `light` minis never take the app compile). `never_touch` hosts are refused.

Acceptance is proved once per class and Xcode build, not per machine. `classes.<class>.acceptance` records it (`xcode_build`, `candidate`, the `node` that holds the receipt, and the exported class receipt as `receipt`, a file on the operator Mac, with its `receipt_sha256`). By default (`--acceptance class`) a host joins only when its class has an acceptance recorded for the declared Xcode build and the current candidate. With a `receipt` recorded, `onboard` checks the file's `receiptSha256` against the manifest, sends it over ssh stdin to `glaeda-mini-enroll --class-receipt - --class-receipt-sha256 SHA --fleet-class CLASS`, and the host becomes `eligible` on a bootstrap re-check, no 13-minute `accept-local` (see [Class acceptance](#class-acceptance)), then is registered. Without a `receipt` it enrolls with `--no-accept` and stays `enrolling`. `--acceptance node` runs `accept-local` on the host instead and registers it. `onboard` exits 1 while any host is not registered.
...
python3.13 -B "$G/scripts/cmux_fleet.py" export-class-acceptance "$F/enrollment.json" \
  --acceptance "$F/acceptance/cmux_macos_native_build.json" --fleet-class std \
