...
Acceptance is proved once per class and Xcode build, not per machine. `classes.<class>.acceptance` records it (`xcode_build`, `candidate`, and the `node` that holds the receipt). By default (`--acceptance class`) a host joins only when its class has an acceptance recorded for the declared Xcode build and the current candidate, and enrolls with `glaeda-mini-enroll --no-accept`: bootstrap and enroll, no 13-minute `accept-local`. `cmux_fleet.py` still binds a receipt to one node id and enrollment generation, so such a host stays `enrolling` and is not registered until `cmux_fleet.py` accepts a class receipt for the eligible transition. `--acceptance node` runs `accept-local` on the host instead, which is today's path to an eligible node, and registers it. `onboard` exits 1 while any host is not registered, including a class-joined host left `enrolling`.

Each host names its `hardware`, a key into the manifest's `hardware` table (model, chip, CPUs, memory), which `check` verifies against the machine. Its scheduling `class` (`xl`, `std`, `light`, `dev`, `borrowed`) says what work it takes, and `availability` (`dedicated`, the default, or `opportunistic`) says whether it can be counted on. A manifest written before this split, with a top-level `classes` table and `class` naming the hardware, still loads: those become `hardware`, and the host has no scheduling class until it is given one.

`glaeda-mini-fleet pools --json` prints runner capacity per pool label `glaeda-<class>-xcode-<version>`, the label glaeda-cmux-runner issues. A member is declared for a pool when it is dedicated, has the `ci-runner` role and a runner class (`xl`, `std`, `light`); it conforms when the latest observation shows that exact Xcode build as a real directory. The PR pool picker counts conforming members.
