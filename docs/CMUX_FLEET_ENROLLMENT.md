...
- `secrets`: the paths (never the contents) of secret files the login user can read: `/Users/Shared/cmux-build-fleet/secrets/*`, `~/.config/glaeda/*.key`, `~/.secrets/*`, `~/Library/Application Support/cmux-build-fleet/secrets/*`, `/Library/Application Support/cmux-build-controller/secrets/*`, `~/.config/gh/hosts.yml` when it holds an `oauth_token`, and `~/.ssh/id_*` keys. Every PR job on a runner host runs as that user, so on a host with a registered runner or the `ci-runner` role any of them is `FAIL` for a person: make them root-only or move them to a host with no PR runners (cmuxterm-hq#595). On a host with no PR runners (an hq builder such as cmux15, whose worker reads its own tokens as that user) they are listed as info.
