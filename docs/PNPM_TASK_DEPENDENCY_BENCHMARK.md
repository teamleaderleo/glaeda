...
The task workspace itself never becomes a shared mutable cache. The closed benchmark environment sets `PNPM_CONFIG_VIRTUAL_STORE_TYPE=project`, which pnpm 12.5.1 applies after project configuration, and the install pins `--virtual-store-dir=node_modules/.pnpm`. Together they keep the virtual store in the task-local project layout even when repository configuration requests the machine-global mode. The resident pnpm store is dependency state and remains a separate mechanism from task source and build output. The measured install enables pnpm's frozen-store mode so the shared store receives no writes during the task-import window.
