...
The benchmark runs install, first-command, and optional probe from the repository root. A monorepo remains bound by the exact Git tree plus the recorded root `package.json`, lockfile, `pnpm-workspace.yaml`, and `.npmrc` digests. The command line deliberately overrides any custom virtual-store location with the task-local `node_modules/.pnpm` treatment. Repositories that require a different virtual-store layout or a package-scoped probe need a separate explicit treatment rather than silently changing this matrix.
