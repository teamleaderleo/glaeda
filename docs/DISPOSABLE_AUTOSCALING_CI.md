...
Add the small durable lifecycle that joins one scale-set capacity claim, one capacity reservation, one VM identity, one runner registration, and the actual GitHub job when GitHub assigns it. Its pure reconciler covers clone authorization, clone start, and exact observed-VM identity binding as separate durable checkpoints so a controller crash cannot replay creation, adopt a same-name replacement, or manufacture deletion authority. The identity is only a durable drift token: the mutation service must still retain and freshly reconfirm the existing descriptor-bound Lima host observation immediately before a command. The reconciler emits one next action: checkpoint-and-clone, bind identity, register, wait, destroy, deregister, release, retry with bounded backoff, or stop for operator policy. Enforce host-wide concurrency and memory/disk ceilings before provisioning.
