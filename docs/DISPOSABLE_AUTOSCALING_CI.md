...
The canonical source-generation document and pure recovery planner now require revision-bound sealed observations and produce only opaque advisory candidates before create, stop, or discard. A candidate is never executable authority: the future same-lock service must obtain a second fresh private observation and reconfirm the complete source, object, template, state, and prior-operation evidence immediately before checkpoint and command. A started external operation is ambiguous until that future supervisor proves it quiescent; it is never blindly replayed after restart. The next implementation boundary is its same-lock Unix persistence plus a private source observer and bounded Lima executor: create the uniquely named source VM from those exact bytes under a private controller-owned `LIMA_HOME` that refuses global default or override files, start it through provisioning, verify the fixed readiness marker and stopped state, and publish ownership only after the full sequence is durably checkpointed. That boundary must delete and rebuild an incomplete source instead of adopting a name or partial install.
