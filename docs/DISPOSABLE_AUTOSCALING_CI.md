...
The canonical source-generation document and pure recovery planner now require revision-bound sealed observations and produce only opaque advisory candidates before create, stop, or discard. Canonical Unix persistence reuses the existing no-follow private store, canonical lock, staged file, exact-successor validation, file and directory fsync, and restart recovery. A persistence transition requires the advisory plan plus a second sealed observation with the exact same source, object, template, state, and prior-operation evidence; command candidates are explicitly rejected. The next implementation boundary is the private source observer and bounded Lima executor that keep the same lock across their own immediate reconfirmation, started checkpoint, and command. A started external operation is ambiguous until that future supervisor proves it quiescent; it is never blindly replayed after restart. The executor will create the uniquely named source VM from the checked-in bytes under a private controller-owned `LIMA_HOME`, verify readiness and stopped state, and delete rather than adopt incomplete construction.
