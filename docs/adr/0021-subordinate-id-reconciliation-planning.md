...
SmolRunner parses each trusted authority snapshot as a whole. Input is capped at one mebibyte, 16,384 records and 4,096 bytes per row. Every row must contain one reviewed account name or canonical positive numeric owner, one canonical positive start, and one canonical nonempty nonoverflowing count.
