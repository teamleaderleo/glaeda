...
| network configuration | can select plugins/helpers or host namespace behavior | Pass `--network=none`; supply one fresh exact run-private network-state directory that is proven empty before first use, writable only because Noble opens `netavark.lock` even for `podman info`, never reused, and removed after group-empty cleanup. Prohibit host, slirp, pasta, bridge, CNI, plugins, DNS helpers, ports, and network namespace joins; any pre-existing entry blocks. |
...
| network state | run-private mutable state | Noble opens `netavark.lock` in the explicit network-config directory during rootless `podman info`, even when no workload network is admitted. Create one protected empty directory writable by the runner for an exact attempt; never treat its later contents as configuration authority or reuse them. Cleanup waits for group emptiness and rejects any pre-existing, rebound, symlinked, special, or unbounded entry. |
...
| network helper | hostile netavark/CNI configuration while command says none | A fresh empty run-private network directory and `--network=none` make persistent config unreachable; only bounded attempt-local state may appear and the marker remains absent. |
