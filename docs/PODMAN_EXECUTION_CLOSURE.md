...
| ELF interpreter and transitive `DT_NEEDED` libraries | replacement changes a reviewed executable before its own checks | Resolve with a non-executing ELF parser or an exact descriptor-bound `/usr/bin/readelf`, never `ldd`; bind the interpreter, every resolved library and hardware-capability variant, `/etc/ld.so.cache`, `/etc/ld.so.preload`, and applicable loader configuration/search directories. Reject preload entries and every `RPATH`. Reject `RUNPATH` except the exact architecture-specific `/usr/lib/<arch>-linux-gnu/systemd` value on the exact `/usr/bin/systemctl` and `/usr/bin/systemd-run` identities; for that exception, descriptor-open and revalidate the root-owned non-writable directory and resolve every library beneath the same complete cache/config/ambiguity policy. Any other executable or runpath, unresolved or ambiguous entry, or user-writable search directory is conflicting. |
