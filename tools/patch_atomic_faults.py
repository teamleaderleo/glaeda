...
marker = 'const TEMP_FILE_PREFIX: &str = ".smolrunner-tmp-";\n'
phases = '''
...
'''
if text.count(marker) != 1 or "enum WritePhase" in text:
    raise SystemExit("unexpected write-phase insertion state")
text = text.replace(marker, marker + phases, 1)

start = text.index("    pub fn write_atomic(\n")
end = text.index("    fn open_required_parent", start)
method = '''    pub fn write_atomic(
        &mut self,
        record: &StateRecord,
...
start = text.index("fn write_and_sync(")
end = text.index("\nfn verify_directory", start)
function = '''fn write_and_sync<F>(
    fd: OwnedFd,
    bytes: &[u8],
    hook: &mut F,
...
old = '''    use crate::state_store::{
        MAX_STATE_DOCUMENT_BYTES, StateRead, StateRecord, StateStoreErrorKind,
        StateWriteDisposition,
    };'''
new = '''    use crate::state_store::{
        MAX_STATE_DOCUMENT_BYTES, StateRead, StateRecord, StateStoreError,
        StateStoreErrorKind, StateWriteDisposition,
    };'''
if text.count(old) != 1:
    raise SystemExit("unexpected state-store test import")
text = text.replace(old, new, 1)

old = "    use super::{LOCK_FILE_NAME, LinuxStateRoot, TEMP_FILE_PREFIX};"
new = '''    use super::{
        LOCK_FILE_NAME, LinuxStateRoot, TEMP_FILE_PREFIX, WritePhase,
    };'''
if text.count(old) != 1:
    raise SystemExit("unexpected linux-state test import")
text = text.replace(old, new, 1)

first_test = text.index("    #[test]\n    fn reads_regular_file_through_canonical_components()")
support = '''    fn fail_at(
        target: WritePhase,
    ) -> impl FnMut(WritePhase) -> Result<(), StateStoreError> {
        move |phase| {
            if phase != target {
                return Ok(());
            }
...
'''
text = text[:first_test] + support + text[first_test:]
...
'''
text = text[:boundary] + tests + text[boundary:]
